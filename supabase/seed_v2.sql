-- Vallavan OTT — Seed Data (matches actual schema on Supabase)
-- Run AFTER schema.sql has been executed

-- =============================================================================
-- DISTRICTS (no sort_order column — just id + name)
-- =============================================================================

INSERT INTO districts (name) VALUES
  ('Chennai'), ('Coimbatore'), ('Madurai'), ('Tiruchirappalli'),
  ('Salem'), ('Tirunelveli'), ('Tiruppur'), ('Vellore'),
  ('Erode'), ('Thoothukudi'), ('Dindigul'), ('Thanjavur'),
  ('Ramanathapuram'), ('Sivaganga'), ('Karur'), ('Kanchipuram'),
  ('Tiruvallur'), ('Nagapattinam'), ('Cuddalore'), ('Viluppuram'),
  ('Namakkal'), ('Dharmapuri'), ('Krishnagiri'), ('Virudhunagar'),
  ('Theni'), ('Nilgiris'), ('Tiruvarur'), ('Ariyalur'),
  ('Perambalur'), ('Pudukkottai'), ('Sivakasi'), ('Kanyakumari'),
  ('Tenkasi'), ('Tirupathur'), ('Chengalpattu'), ('Kallakurichi');
...
-- =============================================================================
-- DOCUMENTARIES (12 items — columns: title, title_ta, genre, duration_sec,
-- poster, backdrop, year, language, synopsis, synopsis_ta, badge, progress,
-- exclusive, director, cast, status, views, sort_order)
-- =============================================================================
...
-- =============================================================================
-- INSPIRE ITEMS (8 items — columns: title, title_ta, category, duration_sec,
-- poster, quote, attribution, badge, sort_order)
-- =============================================================================
...
-- =============================================================================
-- LIVE SLOTS (6 items — columns: title, title_ta, description, thumb,
-- start_time24, duration_min, is_live, air_date, sort_order)
-- =============================================================================

INSERT INTO live_slots (title, title_ta, description, thumb, start_time24, duration_min, is_live, air_date, sort_order) VALUES
  ('Morning Ecology', 'காலை சுற்றுச்சூழல்',
   'Daily bulletin on environmental news across Tamil Nadu.',
   'https://images.pexels.com/photos/30004134/pexels-photo-30004134.jpeg?auto=compress&cs=tinysrgb&w=400',
   '18:00', 30, false, CURRENT_DATE, 0),
  ('Wild Tamil Nadu', 'வனத் தமிழகம்',
   'Live from Anamalai — tracking the tiger population in real time.',
   'https://images.pexels.com/photos/17374833/pexels-photo-17374833.jpeg?auto=compress&cs=tinysrgb&w=400',
   '18:30', 45, true, CURRENT_DATE, 1),
  ('History Uncovered', 'வரலாற்று ரகசியம்',
   'New findings from the Pandya excavation site.',
   'https://images.pexels.com/photos/31969428/pexels-photo-31969428.jpeg?auto=compress&cs=tinysrgb&w=400',
   '19:15', 40, false, CURRENT_DATE, 2),
  ('Science Today', 'இன்றைய அறிவியல்',
   'Latest breakthroughs from Tamil Nadu research labs.',
   'https://images.pexels.com/photos/6325002/pexels-photo-6325002.jpeg?auto=compress&cs=tinysrgb&w=400',
   '19:55', 25, false, CURRENT_DATE, 3),
  ('Delta Stories', 'டெல்டா கதைகள்',
   'Ground report from the Cauvery delta farming community.',
   'https://images.pexels.com/photos/20212135/pexels-photo-20212135.jpeg?auto=compress&cs=tinysrgb&w=400',
   '20:20', 35, false, CURRENT_DATE, 4),
  ('Late Night Investigates', 'இரவு விசாரணை',
   'In-depth investigation into coastal sand mining networks.',
   'https://images.pexels.com/photos/13519711/pexels-photo-13519711.jpeg?auto=compress&cs=tinysrgb&w=400',
   '20:55', 50, false, CURRENT_DATE, 5);

-- =============================================================================
-- APP_USERS (7 items — columns: name, email, role, status)
-- =============================================================================
...
-- =============================================================================
-- SPONSORS (4 items — columns: owner_id, name, email, status)
-- Need to reference app_users IDs, so use a subquery
-- =============================================================================
...
-- =============================================================================
-- CAMPAIGNS (4 + 2 pending — columns: sponsor_id, name, status, impressions,
-- clicks, spend_paise, budget_paise, start_date, submitted_at)
-- =============================================================================
...
INSERT INTO campaigns (sponsor_id, name, status, impressions, clicks, spend_paise, budget_paise, start_date, submitted_at) VALUES
  ...
  ((SELECT id FROM sponsors WHERE name = 'Chennai Motors'), 'Chennai Motors EV Awareness', 'pending', 0, 0, 0, 2500000, '2024-08-10', '2024-08-10'),
  ...
  ((SELECT id FROM sponsors WHERE name = 'A2B Foods'), 'Diwali Sweet Special', 'pending', 0, 0, 0, 1800000, NULL, '2024-08-08'),
  ((SELECT id FROM sponsors WHERE name = 'Mylapore Silks'), 'Brand Launch Teaser', 'pending', 0, 0, 0, 3200000, NULL, '2024-08-05');

-- =============================================================================
-- ADS (4 items — columns: sponsor, sponsor_id, sponsor_logo, headline,
-- body, cta, bg_image, accent, campaign_id)
-- =============================================================================

INSERT INTO ads (sponsor, sponsor_id, sponsor_logo, headline, body, cta, bg_image, accent, campaign_id) VALUES
  ('Tamil Tea Co.',
   (SELECT id FROM sponsors WHERE name = 'Tamil Tea Co.'), '',
   'Pure Nilgiri Tea, Delivered Fresh',
  ...
  ('Chennai Motors',
   (SELECT id FROM sponsors WHERE name = 'Chennai Motors'), '',
   'Drive the Future — Electric Scooters',
  ...
  ('A2B Foods',
   (SELECT id FROM sponsors WHERE name = 'A2B Foods'), '',
   'Authentic Tamil Meals, Anywhere',
  ...
  ('Mylapore Silks',
   (SELECT id FROM sponsors WHERE name = 'Mylapore Silks'), '',
   'Wedding Season Sale — Up to 40% Off',
...
-- =============================================================================
-- AD PLACEMENTS (5 items — columns: sponsor, ad_id, placement, impressions, status)
-- =============================================================================
...
-- =============================================================================
-- FEED REELS (10 items)
-- attached_campaign is a UUID FK — use subquery to get campaign ID
-- =============================================================================
...
INSERT INTO feed_reels (title, title_ta, caption, caption_ta, creator, creator_handle, content_type, genre, duration_sec, thumb, likes, comments, shares, views, status, strip_ad_host, banner_after, attached_campaign, sort_order) VALUES
  ('Cauvery Delta Crisis Update', 'காவிரி டெல்டா நெருக்கடி',
   ...
   'https://images.pexels.com/photos/20212135/pexels-photo-20212135.jpeg?auto=compress&cs=tinysrgb&w=800',
   12400, 340, 890, 145000, 'Published', true, false, NULL, 0),

  ('Tiger Sighting at Anamalai', 'ஆனைமலையில் புலி',
   ...
   'https://images.pexels.com/photos/17374833/pexels-photo-17374833.jpeg?auto=compress&cs=tinysrgb&w=800',
   45200, 1200, 3400, 560000, 'Published', false, false, NULL, 1),

  ('The Man Who Planted 10,000 Trees', 'பத்தாயிரம் மரம் வளர்த்த மனிதன்',
   ...
   'https://images.pexels.com/photos/16983197/pexels-photo-16983197.jpeg?auto=compress&cs=tinysrgb&w=800',
   89000, 2100, 12000, 1200000, 'Published', true, true,
   (SELECT id FROM campaigns WHERE name = 'Nilgiri Tea Summer Push'), 2),

  ('Chola Bronze Technique Revealed', 'சோழ வெண்கல நுட்பம்',
   ...
   'https://images.pexels.com/photos/10899308/pexels-photo-10899308.jpeg?auto=compress&cs=tinysrgb&w=800',
   23100, 560, 1800, 320000, 'Published', false, false, NULL, 3),

  ('ISRO Launch Tracking Live', 'இஸ்ரோ ஏவுதல் நேரடி',
   ...
   'https://images.pexels.com/photos/6325002/pexels-photo-6325002.jpeg?auto=compress&cs=tinysrgb&w=800',
   34200, 890, 2100, 480000, 'Published', true, false, NULL, 4),

  ('Kanchipuram Silk in 60 Seconds', 'காஞ்சிபுரம் பட்டு அறுபது வினாடி',
   ...
   'https://images.pexels.com/photos/12435837/pexels-photo-12435837.jpeg?auto=compress&cs=tinysrgb&w=800',
   56700, 1500, 4500, 780000, 'Published', false, false, NULL, 5),

  ('From Tea Stall to Tech CEO', 'டீ கடையில் இருந்து சிஇஓ',
   ...
   'https://images.pexels.com/photos/19747906/pexels-photo-19747906.jpeg?auto=compress&cs=tinysrgb&w=800',
   102000, 3400, 18000, 2100000, 'Published', true, false, NULL, 6),

  ('Mangrove Loss: The Numbers', 'சதுப்பு நில இழப்பு: எண்கள்',
   ...
   'https://images.pexels.com/photos/30004134/pexels-photo-30004134.jpeg?auto=compress&cs=tinysrgb&w=800',
   18900, 430, 980, 234000, 'Published', false, false, NULL, 7),

  ('Coastal Erosion Investigation Clip', 'கடல் அரிப்பு விசாரணை',
   ...
   'https://images.pexels.com/photos/13519711/pexels-photo-13519711.jpeg?auto=compress&cs=tinysrgb&w=800',
   67000, 2800, 8900, 950000, 'Published', true, true,
   (SELECT id FROM campaigns WHERE name = 'Chennai Motors EV Awareness'), 8),

  ('The Math Whiz Kids Story', 'கணித மேதைகள் கதை',
...
-- =============================================================================
-- PRICING CONFIG (default values — placeholder until confirmed)
-- =============================================================================

INSERT INTO pricing_config (base_post_paise, per_district_paise, bill_dual_as_single) VALUES
  (50000, 5000, true);
